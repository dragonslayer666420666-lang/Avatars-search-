...
if (!fs.existsSync(requestPath)) throw new Error(`${requestPath} is missing.`);
...
const system = `You are the Avatar Search Construction Bot. Produce a minimal, reviewable exact-replacement plan for index.html. Preserve unrelated features. Never add secrets, credentials, analytics, tracking, hidden downloads, remote control, eval, new Function, document.write, or obfuscated code. Keep Android Chrome and phone-only use working. Do not auto-merge. Return JSON only.`;

const user = `APPROVED REQUEST
${request}
...
- each find must be exact text from index.html
...
try {
  ...
  throw new Error(`GitHub Models returned invalid JSON: ${error.message}`);
}
